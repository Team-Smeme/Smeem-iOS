...
struct BookmarkDetailView: View {
        ...
        .toolbar {
            ...
            ToolbarItem(placement: .navigationBarTrailing) {
                ...
                .fullScreenCover(isPresented: $showEditSheet) {
                    ExpressionLearningView()
                    //                                BookmarkEditView(id: self.bookmarkId,
                    //                                                 expression: self.model?.expression,
                    //                                                 translatedExpression: self.model?.translatedExpression,
    ...
    func trimText(_ text: String) -> String {
        if let start = text.firstIndex(of: "\""),
           ...
           start < end {
            let extracted = text[text.index(after: start)..<end]
            return String(extracted)
        }
        return text // 혹시 따옴표 없으면 원문 그대로
    }
